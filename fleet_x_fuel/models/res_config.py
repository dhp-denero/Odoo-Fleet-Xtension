...
class fleet_config_settings(models.TransientModel):
    ...
    def get_default_coupon_creation(self, fields):
        ...
        res = {
            'default_coupon_creation': safe_eval(IrConfigParam.get_param('fleet_x_fuel.default_coupon_creation', 'False') or False),
            'default_price_per_lt': safe_eval(IrConfigParam.get_param('fleet_x_fuel.default_price_per_lt', 'False') or 0.0),
            'default_efficiency_alert_buffer': safe_eval(IrConfigParam.get_param('fleet_x_fuel.default_efficiency_alert_buffer', 'False') or 0.0),
        }
        return res
    ...
    def set_default_coupon_creation(self):
        self.env["ir.config_parameter"].set_param(
            'fleet_x_fuel.default_coupon_creation',
            repr(self.default_coupon_creation)
        )
        self.env["ir.config_parameter"].set_param(
            "fleet_x_fuel.default_price_per_lt",
            self.default_price_per_lt or 0.0
        )
        self.env["ir.config_parameter"].set_param(
            "fleet_x_fuel.default_efficiency_alert_buffer",
